...
namespace NSMBe5
{
    class ImageTiler
    {
        ...
        public ImageTiler(Bitmap b, Size bitmapSize, Size tilemapSize, int mergeDiffPercentage)
        {
            ProgressWindow p = new ProgressWindow(LanguageManager.Get("BgImport", "Importing"));
            p.Show();

            diffSize = mergeDiffPercentage / 100f;
            numberOfCharacters = (bitmapSize.Width * bitmapSize.Height) / 64;
            widthTileCount = tilemapSize.Width / 8;
            heightTileCount = tilemapSize.Height / 8;
            tileCount = widthTileCount * heightTileCount;

            p.SetMax(tileCount);
            tileMap = new int[widthTileCount, heightTileCount];
            ...
            int tileNum = 0;
            for (int xt = 0; xt < widthTileCount; xt++)
            {
                for (int yt = 0; yt < heightTileCount; yt++)
                {
...
//            p.WriteLine("Tiles merged in first pass: " + (64 * 64 - countUsedTiles()) + " of " + 64 * 64);
            ...
            int mustRemove = used - numberOfCharacters;
            if (used > numberOfCharacters)
            {
                p.setValue(0);
                p.SetMax(mustRemove);
            }
            ...
            while (used > numberOfCharacters)
            {
                ...
                used = countUsedTiles();
                p.setValue(mustRemove - used + numberOfCharacters);
            }
